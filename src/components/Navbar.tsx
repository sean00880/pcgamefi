          ...
          <div className="hidden md:flex items-center space-x-8">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.name}
                href={link.path}
                className={`font-semibold transition-colors ${isActivePath(link.path)
          ...
          <div className="hidden md:block relative">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                {/* Tooltip */}
                {showWalletTooltip && (
                  <div className="absolute top-full mt-2 right-0 bg-popover text-popover-foreground px-4 py-3 rounded-lg shadow-xl w-64 z-50 animate-fade-in border border-border">
                    <div className="flex items-start gap-2">
                      <Info size={18} className="flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="font-semibold text-sm mb-1">Coming in Phase 2</p>
                        <p className="text-xs text-muted-foreground">Full Web3 wallet integration with MetaMask, WalletConnect, and more will be available soon.</p>
                      </div>
                    </div>
                    {/* Arrow */}
          ...
          <div className="px-4 py-6 space-y-4">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className={`block py-2 font-semibold text-lg ${isActivePath(link.path) ? 'text-primary' : 'text-foreground'
            ...
            <div className="pt-4 border-t border-border">
              <div className="bg-muted/50 rounded-lg p-4 border border-border">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">Wallet Connection</p>
                </div>
                <p className="text-xs text-muted-foreground">Coming in Phase 2 - Full Web3 integration will be available soon.</p>
              </div>
            </div>
